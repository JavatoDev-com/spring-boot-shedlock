-- SQL Script to generate 10,000 report records
-- This script creates realistic test data for the user_report_data table
...
SELECT 
    ...
    CONCAT(
        'Report data for ', 
        CASE 
            WHEN n % 10 = 0 THEN 'Monthly Sales'
            WHEN n % 10 = 1 THEN 'Weekly Analytics'
            WHEN n % 10 = 2 THEN 'Daily Metrics'
            WHEN n % 10 = 3 THEN 'Quarterly Summary'
            WHEN n % 10 = 4 THEN 'Annual Report'
            WHEN n % 10 = 5 THEN 'Customer Analysis'
            WHEN n % 10 = 6 THEN 'Product Performance'
            WHEN n % 10 = 7 THEN 'Marketing Campaign'
            WHEN n % 10 = 8 THEN 'Financial Report'
            ELSE 'Operational Metrics'
        END,
        ' - Generated on ', 
        DATE_FORMAT(NOW() - INTERVAL (n % 30) DAY, '%Y-%m-%d %H:%i:%s'),
        ' - Record ID: ', n,
        ' - Data: {',
        '"sales_amount": ', ROUND(RAND() * 100000, 2),
        ', "customer_count": ', FLOOR(RAND() * 1000),
        ', "conversion_rate": ', ROUND(RAND() * 100, 2),
        ', "revenue": ', ROUND(RAND() * 50000, 2),
        '}'
    ) as report_data,
    CASE 
...
-- Display report type distribution
SELECT 
    report_type,
    COUNT(*) as count,
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM user_report_data), 2) as percentage
